  ...
  game() {
    setInterval(async () => {
      ...
      actors.forEach(async (actor) => {
        ...
        // Implement action logic based on nextAction
        // Example:
        // if (nextAction) { 
        //   // Execute action based on direction, actionType, etc.
        // }
      });
    }, SPEX.HEARTBEAT);
  }
...
async function createActor(actorname: string) {
  ...
  try {
    ...
    await writeContentToMapTile(x, y, { subject: 'actor', id: ret.id });
...
async function writeContentToMapTile(x: number, y: number, contentData: MapContent): Promise<void> {
  try {
    ...
    console.error(`Error writing content to tile ${x},${y}:`, error);
  }
}
...
async function createPrismaMapFromFile(filePath: string): Promise<void> {
  try {
    ...
    for (let iy = 0; iy < mapLines.length; iy++) {
      ...
      await prisma.mapTile.createMany({ data: mapLineDatasets });
    }
  } catch (error) {
    console.error('Error creating map from file:', error);
  }
}
